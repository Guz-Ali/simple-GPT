...
class GPT(nn.Module):
    ...
    def _init_weights(self, module):
        # based on original GPT 2 initialization parameters
        # other init params / weights in model can stay the same.
    ...
    def from_pretrained(cls, model_type):
        # copied directly from tutorials
    ...
    def configure_optimizers(self, weight_decay, learning_rate, device_type):
        # note every parameters that will be grad
        param_dict = {pn: p for pn, p in self.named_parameters()}
        param_dict = {pn: p for pn, p in param_dict.items() if p.requires_grad}
        # create optim groups. any 2D parameters will have weight decay, otherwise no.
        # weight tensors in matmuls, embeddings: yes. biases and layernorms: no. 
...
if ddp:
    ...
    master_process = ddp_rank == 0 # master proc does logging, checkpoints etc.
else:
    # vanilla, non-DDP run
    ddp_rank = 0
    ddp_local_rank = 0
...
def get_lr(it):
    ...
    # decay done, use min learning rates
    if it > max_steps:
        return min_lr
    # in between: decay area, learning rates follow cosine decay down to min lr
    # GPT 2 and 3, mentioned in GPT3 paper. 
...
for step in range(max_steps):
    ...
    # evaluate validation loss each 100 steps
    if step % 100 == 0:
        model.eval()
        val_loader.reset()
        with torch.no_grad()
